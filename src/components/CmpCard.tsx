...
interface CmpCardProps {
  /** Comparison item with numA, numB, qType. */
  item: Item;
  /** Callback to speak the question. */
  onSpeak: () => void;
}
 ...
 */
export function CmpCard({ item, onSpeak }: CmpCardProps) {
    ...
    <React.Fragment>
      ...
      {showStmt && (
        <div style={{ fontSize: 22, fontWeight: 900, color: '#333', marginBottom: 6 }}>
          ...
          <span style={{ color: '#667eea' }}>
            {item.qType === 'equal' ? 'gleich' : 'größer als'}
          </span>{' '}
          {item.numB}?
        </div>
      )}
      ...
      >
        <div style={{ textAlign: 'center' }}>
          <DotGrid count={item.numA ?? 0} color="#667eea" />
        ...
        <div style={{ textAlign: 'center' }}>
          <DotGrid count={item.numB ?? 0} color="#f5576c" />
