...
import { Owl, ShapeSVG, CmpCard, CmpAnswers } from '../components';
...
export function QuizScreen({
  ...
  const isChoice = cat && cat.type === 'choice';
        ...
        <div style={{ ...CARD, marginBottom: 14, cursor: 'default' }}>
          ...
          {isMth && (
            <>
              ...
              <div style={{ fontSize: 13, color: '#888', marginTop: 4 }}>
                {toG(item.numA ?? 0)} {item.op === '+' ? 'plus' : 'minus'} {toG(item.numB ?? 0)}
              </div>
            ...
            <>
              ...
              >
                {Array.from({ length: item.count || 1 }, (_, i) => (
                  <span key={i} style={{ fontSize: (item.count ?? 0) > 1 ? 44 : 72 }}>
                    {item.emoji}
                  </span>
                ))}
              </div>
          ...
          {isChoice && (
            <>
              ...
              {item.prompt && (
                <div style={{ fontSize: 30, fontWeight: 900, color: '#333', marginBottom: 4 }}>
                  {item.prompt}
                </div>
              )}
              {item.question && (
                <div style={{ fontSize: 14, color: '#555', marginTop: 4, fontWeight: 600 }}>
                  {item.question}
                </div>
              )}
              {item.english && (
          ...
          {!isCF && !isCmp && !isMth && !isPl && !isEO && !isChoice && (
            <>
              <div
                style={{
