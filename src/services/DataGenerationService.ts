...
export const DataGenerationService = {
   ...
   * @returns Array of number items with display, german, english
   */
  genNumbers(): Item[] {
    return shuffle(
      Array.from({ length: 100 }, (_, i) => i + 1)
    ).slice(0, 20).map((n: number): Item => ({
      display: String(n),
      german: toG(n),
      english: String(n)
    }));
   ...
   * @param r - Correct result
   * @returns Array of 4 option strings
   */
  mkOpts(r: number): string[] {
   ...
   * @returns Array of math items
   */
  genMath(): Item[] {
    return Array.from({ length: 20 }, (_, i): Item => {
      let a: number, b: number, op: '+' | '-', r: number;
      if (i % 2 === 0) {
  ...
  genComp(): Item[] {
    const o: Item[] = [];
    for (let i = 0; i < 20; i++) {
      let a: number, b: number, qType: 'bigger' | 'smaller' | 'truefalse' | 'equal', ans: string;
      const t = i % 4;
   ...
   * @returns Array of even/odd items
   */
  genEvenOdd(): Item[] {
    return shuffle(
   ...
   * @returns Array of items for the category
   */
  getItems(c: Cat): Item[] {
    ...
    const its: Item[] = c.dynamic ? this.genNumbers() : shuffle(c.items);
...
export const genNumbers = (): Item[] => DataGenerationService.genNumbers();
export const mkOpts = (r: number): string[] => DataGenerationService.mkOpts(r);
export const genMath = (): Item[] => DataGenerationService.genMath();
export const genComp = (): Item[] => DataGenerationService.genComp();
export const genEvenOdd = (): Item[] => DataGenerationService.genEvenOdd();
export const getItems = (c: Cat): Item[] => DataGenerationService.getItems(c);
